"""Tallukkabot - shared bot logic (persona, commands, keyword replies).

Used by both entry points:
- tallukkabotti.py - local/dev polling entry point.
- api/webhook.py - Vercel serverless webhook entry point.
"""
...
# --------------------------------------------------------------------------
# Application factory (shared by the polling and webhook entry points)
# --------------------------------------------------------------------------


def build_application() -> Application:
    """Build the Application with every handler registered.

    Does not start polling or scheduling - the caller decides how the
    application is run (long-polling locally, or process_update() per
    request behind a webhook).
    """
    if not TOKEN:
        raise RuntimeError(
